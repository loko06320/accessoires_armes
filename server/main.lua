...
function LoadShop()
	...
	local shopResult = MySQL.Sync.fetchAll('SELECT * FROM accessoiresshops')
...
ESX.RegisterServerCallback('esx_accessoires_armes:requestDBItems', function(source, cb)
	if not hasSqlRun then
...
RegisterServerEvent('esx_accessoires_armes:buyItem')
AddEventHandler('esx_accessoires_armes:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('esx_accessoires_armes: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
